...
cuenta_repeticiones:														@;		   |
		...
		mla r9, r1, r5, r2
		ldrb r8, [r0,r9]
		and r10, r8, #MASK_VALOR_SENSE_GEL			@;apliquem màscara per quedarnos amb els 3 bits de menor pes
		cmp r3, #1 									@;analitzem la orientació
		bgt .Nord_Oest 								@;si r3>1 salta (ja que podria ser 2->oest o 3->nord)
		beq .Sud 									@;si r3==1, salta a Sud
		
		.Est: 										@;r3 no és ni 1 ni és més gran que 1, per tant 0 (Est)
		sub r7, r5, #1 								@;r7=COLUMNS-1=8 (per a comparar dins de la posició màxima de la matriu)
		cmp r2, r7 									@;comparem la columna actual amb el màxim de columnes de la matriu
		bge .Fi 									@;si r2>=r7 (si la columna actual es superior o igual a la màxima), acaba el programa
		...
		b .Est										@;torna a començar el bucle del recorregut
		
		.Sud: 										@;r3 és 1
		sub r7, r6, #1 								@;r7=ROWS-1=8 (per a comparar dins de la posició màxima de la matriu)
		cmp r1, r7 									@;comparem la fila actual amb el màxim de columnes de la matriu
		bge .Fi 									@;si r1>=r7 (si la fila actual es superior o igual a la màxima), acaba el programa
		...
		bne .Fi										@;si son diferents, s'acaba el recorregut i el programa
		add r4,#1									@;si els dos valors son iguals, s'incrementa el número de repeticions consecutius
		b .Sud										@;torna a començar el bucle del recorregut
		
		.Nord_Oest:									@;salt a estudiar possible nord o oest
		cmp r3, #2 									@;comparem la orientació amb el valor 2
		beq .Oest									@;si r3==2, té orientació de oest, sinò serà nord
		
		.Nord: 										@;si no es oest, serà nord
		cmp r1, #0 									@;comparem amb la posició mínima de la matriu
		ble .Fi										@;si r1<=0 (si la fila actual es inferior o igual a la mínima), acaba el programa
		...
		bne .Fi										@;si son diferents, s'acaba el recorregut i el programa
		add r4,#1									@;si els dos valors son iguals, s'incrementa el número de repeticions consecutius
		b .Nord										@;torna a començar el bucle del recorregut
			
		.Oest:										@;r3 és 2 (oest)
		cmp r2, #0 									@;comparem amb la posició mínima de la matriu
		ble .Fi 									@;si r2<=0 (si la columna actual es inferior o igual a la mínima), acaba el programa
		...
		bne .Fi										@;si son diferents, s'acaba el recorregut i el programa
		add r4,#1									@;si els dos valors son iguals, s'incrementa el número de repeticions consecutius
		b .Oest										@;torna a començar el bucle del recorregut
